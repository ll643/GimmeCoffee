...
class Construction_Cost:
  ...
  def rapid_sand_cc(self):
    if self.x >= 1*(u.Mgal/u.day) and self.x <= 200*(u.Mgal/u.day):
      ...
      print('Plant flow rate is outside of range.')
      return(-1)
  ...
  def backwash_cc(self):
    if self.p >=1.8*(u.gal/u.min) and self.p <= 33*(u.gal/u.min):
      ...
      print('Pumping capacity is outside of range.')
      return(-1)
  ...
  def total_cc(self):
    ...
    if pretreatment_cc2 == -1 or coagulation_cc == -1 or rapid_mix_cc == -1 or flocculator_cc == -1 or clarifier_cc == -1 or gravity_cc == -1 or rapid_sand_cc == -1 or backwash_cc == -1 or hydraulic_sa_cc == -1 or wash_water_surge_cc == -1 or wash_water_storage_cc == -1 or management_cc == -1 or storage_cc == -1:
      print('Total cost could not be computed because a parameter is outside the range')
    else:
...
print('The total construction costs of the plant are', ut.round_sf(Cost_c_sample.total_cc(),6))
...
print ('The total plant operation and management cost estimation is', ut.round_sf(Total_oc_sample,4), '.')
...
print('The total operational and maintenance costs of the plant are', ut.round_sf(total_cost_o_sample,3))
...
print ('The Total Plant Construction Cost Estimation is', ut.round_sf(Total_Ithaca_cc,4), '.')
...
print('The total construction costs of the plant are', ut.round_sf(total_ith_cost_c,6))
# simple operation
Total_Ithaca_oc = simple_OC(c_ith, f_ith, v_ith, fa_ith, x_ith, s_ith, p_ith)
print ('The total plant operation and management cost estimation is', ut.round_sf(Total_Ithaca_oc,4), '.')
# complex operation
Cost_Ithaca_o = Operation_Cost(c_ith, f_ith, v_ith, fa_ith, x_ith, s_ith, p_ith)
total_ith_cost_o =  Cost_Ithaca_o.total_oc()
print('The total operational and maintenance costs of the plant are', ut.round_sf(total_ith_cost_o,3))
```

# Calculating the Minimum Cost of Construction and Operation for a Water Treatment Plant
```Python
# x = plant flow rate, plant capacity (mgd)
...
print('The minimum total construction costs of the plant are', ut.round_sf(Min_cost_complex,6))
# simple operation
Total_oc_min = simple_OC(c_min, f_min, v_min, fa_min, x_min, s_min, p_min)
print ('The total minimum plant operation and management cost estimation is', ut.round_sf(Total_oc_min,4), '.')
# complex operation
Cost_o_min = Operation_Cost(c_min, f_min, v_min, fa_min, x_min, s_min, p_min)
total_cost_o_min =  Cost_o_min.total_oc()
print('The total minimum operational and maintenance costs of the plant are', ut.round_sf(total_cost_o_min,3))
```

# Calculating the Maximum Cost of Construction and Operation for a Water Treatment Plant
```Python
# x = plant flow rate, plant capacity (mgd)
...
print('The maximum total construction costs of the plant are', ut.round_sf(Max_cost_complex,6))
# simple operation
Total_oc_max = simple_OC(c_max, f_max, v_max, fa_max, x_max, s_max, p_max)
print ('The total maximum plant operation and management cost estimation is', ut.round_sf(Total_oc_max,4), '.')
# complex operation
Cost_o_max = Operation_Cost(c_max, f_max, v_max, fa_max, x_max, s_max, p_max)
total_cost_o_max =  Cost_o_max.total_oc()
print('The total maximum operational and maintenance costs of the plant are', ut.round_sf(total_cost_o_max,3))
```

| | Model Ithaca Estimation Cost  |Actual Ithaca Cost| Model Minimum Plant Cost | Model Maximum Plant Cost|
|---:|---|---|---|---|
...
|      Gracias AguaClara Plant |                       | > $1 million (budget)   |                                                                                        |                     |
|     Moroceli AguaClara Plant | 16                    | $200,883                |                                                                                        |                     |
